...
<template>
  <div class="min-h-screen bg-[var(--bg)] text-[var(--text)]">
    ...
    <div v-if="!isAuthenticated" class="min-h-screen flex items-center justify-center p-6">
      <div class="bg-[var(--surface)] text-[var(--text)] border border-[var(--border)] rounded-lg shadow-xl max-w-md w-full p-8">
        ...
        <form @submit.prevent="handleLogin" class="space-y-4">
          <div>
            <label class="block text-sm font-medium mb-2 text-[var(--text)]">
              Mot de passe
            </label>
            <input 
              v-model="password"
              ...
              class="w-full px-4 py-3 rounded-lg border-2 border-[var(--border)] bg-[var(--surface)] text-[var(--text)] focus:ring-2 focus:ring-[var(--ring)] focus:border-transparent"
              :disabled="loading"
              autofocus
            />
          </div>
          ...
          <button 
            type="submit"
            :disabled="loading"
            class="w-full px-4 py-3 rounded-lg bg-[var(--accent)] text-white hover:bg-[var(--accent-hover)] disabled:opacity-50 disabled:cursor-not-allowed transition-colors font-medium flex items-center justify-center gap-2"
          >
        ...
        <div class="mt-6 text-center">
          <button 
            @click="router.push('/')"
            class="text-sm text-[var(--text)] hover:opacity-80 transition-colors"
          >
            ← Retour à l'accueil
          </button>
        </div>
    ...
    <div v-else class="min-h-screen">
      <!-- Header -->
      <header class="sticky top-0 z-40 bg-[var(--surface)] border-b border-[var(--border)]">
        <div class="max-w-7xl mx-auto px-6 py-4">
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-4">
              <button 
                @click="router.push('/')"
                class="px-4 py-2 rounded-lg transition-colors text-[var(--text)] hover:bg-[var(--surface-alt)] border border-[var(--border)]"
              >
                ← Accueil
              </button>
              <button 
      ...
      <div v-if="activeTab === 'questions'" class="max-w-7xl mx-auto px-6">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div class="bg-[var(--surface)] text-[var(--text)] border border-[var(--border)] rounded-lg shadow p-6">
            <div class="flex items-center justify-between">
              <div>
          ...
          <div class="bg-[var(--surface)] text-[var(--text)] border border-[var(--border)] rounded-lg shadow p-6 md:col-span-2">
